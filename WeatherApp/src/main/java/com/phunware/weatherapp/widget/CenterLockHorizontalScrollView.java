package com.phunware.weatherapp.widget;
...
public class CenterLockHorizontalScrollView extends HorizontalScrollView {
    ...
    public void setAdapter(Context context, ForecastAdapter mAdapter) {

        try {
            fillViewWithAdapter(mAdapter);
        } catch (Exception e) {

            Log.v("DEBUG", "fillViewWithAdapter threw : " + e.toString());
        }
    }

    private void fillViewWithAdapter(ForecastAdapter mAdapter)
            throws Exception {
        if (getChildCount() == 0) {
            throw new Exception(
